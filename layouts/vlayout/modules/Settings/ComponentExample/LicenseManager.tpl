    ...
    <form method="POST" action='#' onsubmit="storeLicense();return false;">
        <div class="widget_header row-fluid">
            <div class="span12">
                <h3>
                    <b>
                        {vtranslate('ComponentExample', 'ComponentExample')} {vtranslate('License Configuration', 'ComponentExample')}
                    </b>
                </h3>
            </div>
        </div>
        ...
        <div style="margin:20px auto;width:400px;">
            <h4>{vtranslate('Enter your license code', 'ComponentExample')}</h4>
            <hr/>
            ...
            <input type="button" class="btn btn-primary" id="setlicenseBtn" onclick="storeLicense();" value="{vtranslate('store license', 'ComponentExample')}" />
        </div>
        {else}
            <table class="table table-bordered  table-condensed" style="width:500px;">
                <tr class="{if $ACTIVE_LICENSE eq true}success{else}error{/if}">
                    <td width="200">
                        {vtranslate('LBL_LICENSE_IS', 'ComponentExample')}
                    </td>
                    <td>
                        <strong>
                            {if $ACTIVE_LICENSE eq true}
                                {vtranslate('LBL_ACTIVE','ComponentExample')}
                                {else}
                                {vtranslate('LBL_INACTIVE','ComponentExample')}
                            {/if}
                        </strong>
                    </td>
                </tr>
            ...
            <button class="btn btn-danger" onclick="removeLicense();">{vtranslate('remove License', 'ComponentExample')}</button>
        {/if}
    </form>
</div>
