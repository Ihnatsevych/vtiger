...
class ModComments_Save_Action extends Vtiger_Save_Action {
	...
	protected function getRecordModelFromRequest(Vtiger_Request $request) {
		...
		if($request->has('customer')) {
			$recordModel->set('customer', intval($request->getRaw('customer')));
		} else {
			$recordModel->set('customer', 0);
		}
		if($request->has('parent_comments')) {
			$recordModel->set('parent_comments', intval($request->getRaw('parent_comments')));
		} else {
			$recordModel->set('parent_comments', 0);
		}
