...
class Settings_ComponentExample_Upgrade_View extends Settings_Vtiger_Index_View {
    public function process(Vtiger_Request $request) {

        $moduleName = $request->getModule();
        $moduleClassName = 'ComponentExample';

        $className = '\\'.$moduleName.'\\Autoload';
        $className::registerDirectory("~/modules/".$moduleName."/lib");

        $className = '\\'.$moduleClassName.'\\SWExtension\\AutoUpdate';
        $objUpdater = new $className($moduleName, "beta");
...
?>
    <div class="listViewPageDiv">
    	<div class="listViewTopMenuDiv">
            <div class="row-fluid">
        		<label class=" pull-left themeTextColor font-x-x-large">Upgrade <?php echo getTranslatedString($moduleName, $moduleName) ?> in vtigerCRM <?php echo $vtiger_current_version ?> | Step <?php echo $step; ?></label>
            </div>
            <hr>
    		<div class="clearfix"></div>
    	</div>
    	<div class="listViewContentDiv" id="listViewContents">
            <div style="width:500px;min-height:200px;background-color:white;border:1px solid #000;box-shadow:1px 1px 5px #aaa;padding:10px;">
            <?php
                if($step == 1) {
                    ...
                    echo "<div style='font-size:15px;'>Current available version: ".(!empty($latestVersion)?$latestVersion:'<em>not available</em>')."</div>";
