 ...
 *  # 1.24
 *    Fixed a bug where mathematical calculations are executed wrong
 *	# 1.23
...
//        $code = html_entity_decode(htmlspecialchars_decode($code, ENT_NOQUOTES), ENT_NOQUOTES, "UTF-8");
    ...
    {
        $files = glob(vglobal('root_directory') . DS . 'modules' . DS . 'Workflow2' . DS . 'extends' . DS . 'functions' . DS . '*.inc.php');
        $functions = array();
    ...
    private function _nextCommand($doNotLookForward = false)
    {
        $return = "";
        ...
        */
        ...
        switch ($nextToken[0]) {
            ...
            case T_STRING:
                ...
                } elseif ($this->_disableFunctionlist === true ||
        ...
        if ($doNotLookForward !== true && in_array($tmpToken, array("+", "-", "*", "/", "^"))) {
            $this->_debug("    found Operation");

            //$this->_currentPos++;
            $operation = ' ' . $return;

            $operation = $this->getCalculationString($operation);
            //var_dump($operation);
            ...
            $this->_debug("    run Operation ('return ".$operation.";')");
    ...
    private function getCalculationString($start = '', $output = false) {
        $operation = $start;
        $calcValue = null;
        $operator = null;
        $maxLength = 0;
        $startWithOperator = null;
        $output = false;

        do {
            $valueOfVariable = $this->_next(false);

            if($valueOfVariable === -1) {
                break;
            }

            if(is_array($valueOfVariable)) {
                $calcValue = $this->_nextCommand(true);
                if($output) var_dump('set calcValues', $calcValue);
                if($startWithOperator === null) $startWithOperator = false;
            }

            if(is_string($valueOfVariable)) {
                if(in_array($valueOfVariable, array("+", "-", "*", "/", "^"))) {
                    $operator = $valueOfVariable;
                    if($output) var_dump('set operator', $operator);

                    if($startWithOperator === null) $startWithOperator = true;

                    $this->_currentPos++;
                } elseif($valueOfVariable == ")") {
                    if(!empty($calcValue)) {
                        $operation .= (!empty($operator)?$operator:'').' ' . $calcValue;
                    }
                    ...
                    $operationString = $this->getCalculationString('', true);

                    //if($operator !== null) {
                    $calcValue = '('.$operationString.')';
//							$operator = null;
                    //$calcValue = null;
                    //}

                    //
//						$operator = $this->_nextCommand();
                    //var_dump('set operator', $operator);
...
//				$valueOfVariable = ;
            if($output) var_dump('append', $startWithOperator, $operator . ' ' . $calcValue . '');

            if($operator !== null && $calcValue !== null) {
                if($startWithOperator === true) {
                    $operation .= ' '.$operator . ' ' . $calcValue . '';
                } else {
                    $operation .= ' ' . $calcValue . ' '.$operator;
                }
                $calcValue = null;
                $operator = null;
            }
            ...
            if($maxLength++ > 100) {
                throw new \Exception('Calculation to complexe.');
            }
