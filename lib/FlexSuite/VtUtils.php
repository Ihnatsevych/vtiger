...
//                require_once(REPORTS_EXTENDS_DIR . '/SqlFormatter.php');
    ...
    private static function getCryptKeyPath()
    {
        return ROOT_DIRECTORY . DS . 'storage' . DS . 'cryptkey.dat';
    }


    public static function encrypt($value, $key = '')
    {
        $cryptKeyPath = self::getCryptKeyPath();

        if (empty($key)) {
            if (!file_exists($cryptKeyPath)) {
                file_put_contents($cryptKeyPath, self::generate_password(50));
            }

            $key = file_get_contents($cryptKeyPath);
            $traceData = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 2);
            $key .= substr(md5(substr(md5(str_replace(ROOT_DIRECTORY, '', $traceData[0]['file'])), 0, 10)), 0, 5);
        }

        $iv = 'abc123+=';
        $bf = new \phpseclib\Crypt\AES(\phpseclib\Crypt\AES::MODE_CBC);
        $bf->setIV($iv);
        //$bf->paddable = false;
        $bf->setPassword($key);
...
//        return $brightness;
    ...
    {
        if (vtlib_isModuleActive($module_name) === false) {
            return array();
        }
        $adb = \PearDatabase::getInstance();
