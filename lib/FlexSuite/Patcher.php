...
/**SWPATCHER-' . strtoupper(md5($mod['id'])) . '-FINISH**/';
    ...
    public function applyPatchFile($patchFilename, $basePath, $dryRun = true)
    {
        ...
        foreach ($files as $fileName => $modification) {
            try {
                $fileCounter = $this->applyFile($fileName, $modification, $dryRun);
                if ($fileCounter > 0) {
    ...
    public function restorePatchFile($patchFilename, $basePath, $restoreHash, $backupFolder = false)
    {
        ...
        foreach ($filenames as $file) {
            ...
            $this->restoreBackup($file, $restoreHash, $backupFolder);
        }
    }
    private function restoreBackup($filename, $hash, $corruptBackupFolder)
    {
    ...
    private function clearOldModifications($filename, $modifications, $dryRun)
    {
        $changed = false;
        ...
        foreach ($modifications as $index => $mod) {
            if (strpos($content, '/**SWPATCHER-' . strtoupper(md5($mod['id'])) . '-START') !== false) {
                ...
                $content = preg_replace_callback('/\/\*\*SWPATCHER-' . strtoupper(md5($mod['id'])) . '-START-(.*?)\*\*\\/(.*)\/\*\*SWPATCHER-' . strtoupper(md5($mod['id'])) . '-FINISH\*\*\//s', array($this, 'replaceOldModification'), $content);
    ...
    private function replaceOldModification($matches)
    {
...
//        $mod['modification'][] = '';
    ...
    private function applyFile($filename, $modifications, $dryRun)
    {
        $fileCounter = 0;
        ...
        $modifications = $this->clearOldModifications($filename, $modifications, $dryRun);
